<div class="p-6">
    ...
    <div class="flex items-center gap-4 pb-4 border-b border-slate-200">
        ...
        @endif
        <div class="flex-1">
            <h3 class="text-xl font-bold text-slate-900 dark:text-slate-100">{{ $category->name }}</h3>
    ...
    <div class="mt-6 space-y-4">
        ...
        <div class="flex items-center justify-between py-3 border-b border-slate-100 dark:border-slate-800/60">
            <div class="flex items-center gap-2">
                ...
                <span class="text-sm font-medium text-slate-600 dark:text-slate-400">Category ID</span>
            </div>
            <span class="text-sm font-semibold text-slate-900 dark:text-slate-100">#{{ $category->id }}</span>
        </div>
        ...
        <div class="flex items-center justify-between py-3 border-b border-slate-100 dark:border-slate-800/60">
            ...
            <span class="text-sm text-slate-900 dark:text-slate-200">{{ $category->name }}</span>
        </div>
        ...
        <div class="flex items-center justify-between py-3 border-b border-slate-100 dark:border-slate-800/60">
            <div class="flex items-center gap-2">
                ...
                <span class="text-sm font-medium text-slate-600 dark:text-slate-400">Slug</span>
            </div>
            <span class="text-sm text-slate-900 dark:text-slate-100 font-mono bg-slate-50 dark:bg-slate-800/50 px-2 py-1 rounded">{{ $category->slug }}</span>
        </div>
        ...
        <div class="flex items-center justify-between py-3 border-b border-slate-100 dark:border-slate-800/60">
            ...
            @if ($category->parent)
                <span class="text-sm text-slate-900 dark:text-slate-200">{{ $category->parent->name }}</span>
            @else
                <span class="text-xs text-slate-400 dark:text-slate-550">Root Category</span>
            @endif
        </div>
        ...
        @if ($category->description)
            <div class="py-3 border-b border-slate-100 dark:border-slate-800/60">
                ...
                <p class="text-sm text-slate-700 dark:text-slate-300 ml-7">{{ $category->description }}</p>
            </div>
        @endif
        ...
        <div class="flex items-center justify-between py-3 border-b border-slate-100 dark:border-slate-800/60">
            ...
            <div class="text-right">
                <p class="text-sm font-medium text-slate-900 dark:text-slate-100">{{ $category->created_at->format('M d, Y') }}</p>
                <p class="text-xs text-slate-500 dark:text-slate-400">{{ $category->created_at->format('h:i A') }}</p>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between py-3 border-b border-slate-100 dark:border-slate-800/60">
            ...
            <div class="text-right">
                <p class="text-sm font-medium text-slate-900 dark:text-slate-100">{{ $category->updated_at->format('M d, Y') }}</p>
                <p class="text-xs text-slate-500 dark:text-slate-400">{{ $category->updated_at->format('h:i A') }}</p>
            </div>
        </div>
        ...
        @if ($category->children->count() > 0)
            <div class="py-3">
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <span class="text-sm font-medium text-slate-600 dark:text-slate-400">Subcategories
                        ({{ $category->children->count() }})</span>
                </div>
                <div class="ml-7 flex flex-wrap gap-2">
                    @foreach ($category->children as $child)
                        <a href="{{ route('categories.show', $child->id) }}"
                            class="inline-flex items-center px-3 py-1 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-full text-xs font-medium transition-colors">
                            {{ $child->name }}
                        </a>
                    @endforeach
                </div>
