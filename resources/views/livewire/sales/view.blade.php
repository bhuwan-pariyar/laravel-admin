<div class="py-2" id="printable-area">
    <div class="bg-white p-8 rounded-1xl border border-slate-200 shadow-sm max-w-4xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:justify-between items-start md:items-center pb-8 border-b border-slate-100 gap-6">
            <div>
                <h2 class="text-2xl font-bold text-slate-800 tracking-tight">INVOICE</h2>
                <p class="text-xs text-slate-500 mt-1">Invoice Number: <strong class="text-slate-800">{{ $sale->invoice_no }}</strong></p>
                <p class="text-xs text-slate-500">Date: <strong class="text-slate-800">{{ $sale->sale_date->format('Y-m-d') }}</strong></p>
            </div>
            <div class="text-right">
                <div class="text-lg font-bold text-blue-600">INVENTORY PORTAL</div>
                <p class="text-xs text-slate-500">Store: <strong class="text-slate-800">{{ $sale->store->name }}</strong></p>
                <p class="text-xs text-slate-500">Location: <strong class="text-slate-800">{{ $sale->store->location }}</strong></p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 py-8 border-b border-slate-100">
            <div>
                <span class="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Billed To</span>
                <div class="text-slate-800 font-bold mt-1.5">{{ $sale->customer->name }}</div>
                <p class="text-xs text-slate-500 mt-1"><i class="fa-solid fa-envelope mr-1 text-slate-400"></i> {{ $sale->customer->email }}</p>
                <p class="text-xs text-slate-500"><i class="fa-solid fa-phone mr-1 text-slate-400"></i> {{ $sale->customer->phone }}</p>
                <p class="text-xs text-slate-500"><i class="fa-solid fa-location-dot mr-1.5 text-slate-400"></i> {{ $sale->customer->address }}</p>
            </div>
            <div class="md:text-right">
                ...
                <div class="mt-2">
                    ...
                    @endphp
                    <span class="px-3 py-1 text-xs font-bold rounded-full {{ $colors[strtolower($sale->payment_status)] ?? 'bg-slate-100' }}">
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-slate-50 border-b border-slate-100">
                        ...
                        <th class="px-4 py-3 text-xs font-semibold text-slate-500 uppercase text-center w-24">Price</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach ($sale->items as $item)
                        <tr>
                            ...
                            <td class="px-4 py-4 text-xs text-slate-600 text-center">${{ number_format($item->selling_price, 2) }}</td>
        ...
        <div class="flex flex-col md:flex-row md:justify-between items-start pt-6 border-t border-slate-100 gap-6">
            ...
            <div class="w-full md:w-72 space-y-2.5">
                ...
                @if ($sale->discount_amount > 0)
                    <div class="flex justify-between text-xs text-slate-600">
                        <span>Discount:</span>
                        <span class="text-emerald-600">-${{ number_format($sale->discount_amount, 2) }}</span>
                    </div>
                @endif
                <div class="flex justify-between text-sm font-bold text-slate-800 pt-2 border-t border-slate-100">
                    <span>Grand Total:</span>
                    <span class="text-blue-600 text-base">${{ number_format($sale->grand_total, 2) }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 mt-10 pt-6 border-t border-slate-100 print:hidden">
            <a href="{{ route('sales.list') }}" class="px-4 py-2 text-xs font-medium text-slate-700 bg-slate-100 rounded hover:bg-slate-200 transition">Back to List</a>
