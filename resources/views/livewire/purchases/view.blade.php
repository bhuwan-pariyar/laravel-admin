<div class="py-2" id="printable-area">
    <div class="bg-white dark:bg-slate-900 p-8 rounded-1xl border border-slate-200 dark:border-slate-800 shadow-sm mx-auto text-slate-800 dark:text-slate-200">
        ...
        <div class="flex flex-col md:flex-row md:justify-between items-start md:items-center pb-8 border-b border-slate-100 dark:border-slate-800/80 gap-6">
            <div>
                <h2 class="text-2xl font-bold text-slate-800 dark:text-slate-100 tracking-tight">PURCHASE ORDER</h2>
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Purchase Number: <strong class="text-slate-800 dark:text-slate-200">{{ $purchase->purchase_no }}</strong></p>
                <p class="text-xs text-slate-500 dark:text-slate-400">Date: <strong class="text-slate-800 dark:text-slate-200">{{ $purchase->purchase_date->format('Y-m-d') }}</strong></p>
            </div>
            <div class="text-right">
                <div class="text-lg font-bold text-blue-600 dark:text-blue-400">INVENTORY PORTAL</div>
                <p class="text-xs text-slate-500 dark:text-slate-400">Store: <strong class="text-slate-800 dark:text-slate-200">{{ $purchase->store->name }}</strong></p>
                <p class="text-xs text-slate-500 dark:text-slate-400">Location: <strong class="text-slate-800 dark:text-slate-200">{{ $purchase->store->location }}</strong></p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 py-8 border-b border-slate-100 dark:border-slate-800/80">
            <div>
                <span class="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-550 tracking-wider">Supplier Details</span>
                <div class="text-slate-800 dark:text-slate-100 font-bold mt-1.5">{{ $purchase->supplier->name }}</div>
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1"><i class="fa-solid fa-envelope mr-1 text-slate-400 dark:text-slate-500"></i> {{ $purchase->supplier->email }}</p>
                <p class="text-xs text-slate-500 dark:text-slate-400"><i class="fa-solid fa-phone mr-1 text-slate-400 dark:text-slate-500"></i> {{ $purchase->supplier->phone }}</p>
                <p class="text-xs text-slate-500 dark:text-slate-400"><i class="fa-solid fa-location-dot mr-1.5 text-slate-400 dark:text-slate-500"></i> {{ $purchase->supplier->address }}</p>
            </div>
            <div class="md:text-right">
                <span class="text-[10px] uppercase font-bold text-slate-400 dark:text-slate-550 tracking-wider">Payment Status</span>
                <div class="mt-2">
                    @php
                        $colors = [
                    ...
                    @endphp
                    <span class="px-3 py-1 text-xs font-bold rounded-full {{ $colors[strtolower($purchase->payment_status)] ?? 'bg-slate-100 dark:bg-slate-800' }}">
            ...
            <table class="w-full text-left">
                <thead>
                    <tr class="bg-slate-50 dark:bg-slate-800/40 border-b border-slate-100 dark:border-slate-800/60">
                        ...
                        <th class="px-4 py-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase text-center w-24">Cost</th>
                    ...
                    @foreach ($purchase->items as $item)
                        <tr>
                            <td class="px-4 py-4 text-xs font-medium text-slate-800 dark:text-slate-200">
                                {{ $item->item->name }}
                                <span class="block text-[10px] text-slate-400 dark:text-slate-550 mt-0.5">SKU: {{ $item->item->sku }}</span>
                            </td>
                            <td class="px-4 py-4 text-xs text-slate-600 dark:text-slate-400 text-center">${{ number_format($item->cost_price, 2) }}</td>
        ...
        <div class="flex flex-col md:flex-row md:justify-between items-start pt-6 border-t border-slate-100 dark:border-slate-800/80 gap-6">
            ...
            <div class="w-full md:w-72 space-y-2.5">
                ...
                @if ($purchase->discount_amount > 0)
                    <div class="flex justify-between text-xs text-slate-600 dark:text-slate-400">
                        <span>Discount:</span>
                        <span class="text-emerald-600 dark:text-emerald-400">-${{ number_format($purchase->discount_amount, 2) }}</span>
                    </div>
                @endif
                <div class="flex justify-between text-sm font-bold text-slate-800 dark:text-slate-200 pt-2 border-t border-slate-100 dark:border-slate-800">
                    ...
                    <span class="text-blue-600 dark:text-blue-400 text-base">${{ number_format($purchase->grand_total, 2) }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 mt-10 pt-6 border-t border-slate-100 dark:border-slate-800 print:hidden">
            <a href="{{ route('purchases.list') }}" class="px-4 py-2 text-xs font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 rounded hover:bg-slate-200 dark:hover:bg-slate-700 transition">Back to List</a>
