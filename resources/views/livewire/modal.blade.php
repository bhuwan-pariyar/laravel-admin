<div>
    @if ($show)
        <div x-data="{ show: @entangle('show').live }" x-show="show" x-cloak x-transition:enter="transition ease-out duration-300"
            ...
            <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm"
                @if ($closeable) wire:click="closeModal" @endif>
            </div>
            ...
            <div class="flex min-h-full items-center justify-center p-2 sm:p-6">
                <div x-transition:enter="transition ease-out duration-300"
                    ...
                    class="relative bg-white rounded-sm shadow-xl w-full transform transition-all
                        @if ($size === 'sm') max-w-sm
                    ...
                    {{-- Header --}}
                    @if ($title)
                        <div class="flex items-center justify-between px-6 py-2 border-b border-slate-200">
                    ...
                    <div class="max-h-[calc(100vh-200px)] overflow-y-auto p-6">
                        @if ($component)
                            @livewire($component, $parameters, key($component . '-' . now()))
                        @endif
                    </div>
